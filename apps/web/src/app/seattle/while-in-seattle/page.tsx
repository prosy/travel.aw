...
function initialIntent(raw?: string) {
  return parseIntentId(raw, "whats_around");
}
...
export default async function WhileInSeattlePage({ searchParams }: SearchProps) {
  ...
  const intentId = initialIntent(params?.intent);
      ...
      <section className="section page-grid">
        <div>
          ...
          <section className="section stats muted">
            ...
            <span className="pill">near={payload.report.near ?? "none"}</span>
        ...
        <aside className="card card-soft sticky-panel">
          ...
          <div className="section grid" style={{ gap: 8 }}>
            <span className="pill">phase=while_in_seattle</span>
            <span className="pill">intent={intentId}</span>
            <span className="pill">near={near}</span>
