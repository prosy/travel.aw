...
function buildHref(intent: (typeof intents)[number]["id"], near: string, limit: number): string {
  const params = new URLSearchParams();
  params.set("intent", intent);
  params.set("near", near);
  params.set("limit", String(limit));
  return `/seattle/while-in-seattle?${params.toString()}`;
}

function buildApiHref(intent: (typeof intents)[number]["id"], near: string, limit: number): string {
  const params = new URLSearchParams();
  params.set("phase", "while_in_seattle");
  params.set("intent", intent);
  params.set("near", near);
  params.set("limit", String(limit));
...
export default async function WhileInSeattlePage({ searchParams }: SearchProps) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <h1 className="text-2xl font-bold">While in Seattle</h1>
      <p className="text-sm text-zinc-500 dark:text-zinc-400">
        In-city mode for what to do now, what is happening now, and what is around your neighborhood.
      </p>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_280px]">
        <div>
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-sm text-zinc-500 dark:text-zinc-400">
            ...
            <span className="inline-block rounded-full bg-zinc-100 px-2.5 py-0.5 text-xs font-medium text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400">near={payload.report.near ?? "none"}</span>
        ...
        <aside className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-900/50 lg:sticky lg:top-6 lg:self-start">
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            Current in-city query context for this page.
          </p>
          <div className="mt-4 flex flex-wrap gap-2">
            <span className="inline-block rounded-full bg-zinc-100 px-2.5 py-0.5 text-xs font-medium text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400">phase=while_in_seattle</span>
            ...
            <span className="inline-block rounded-full bg-zinc-100 px-2.5 py-0.5 text-xs font-medium text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400">near={near}</span>
